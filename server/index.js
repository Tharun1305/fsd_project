...
// ─── CORS ─────────────────────────────────────────────────────────────────────
// Allow the deployed frontend origin(s) plus local dev.
// Set ALLOWED_ORIGINS env var on Render to a comma-separated list of frontend URLs
// e.g. "https://gv-clothings.web.app,https://gv-clothings.firebaseapp.com"
...
const corsOptions = {
  origin: (origin, callback) => {
    ...
    // In development/open mode (no ALLOWED_ORIGINS configured), allow everything
...
// ─── Initialise seed data (runs once if local JSON is empty) ──────────────────
(async () => {
  try {
    ...
    console.warn('Could not check seed data:', e.message);
  }
})();
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    database: isFirestoreConnected ? 'Google Cloud Firestore (Live - Global Shared)' : 'Local JSON Storage (Fallback)',
...
app.get('/api/sample-requests', async (req, res) => {
  try {
    res.json({ success: true, data: await db.getSampleRequests() });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.get('/api/callback-requests', async (req, res) => {
  try {
    res.json({ success: true, data: await db.getCallbackRequests() });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.get('/api/offers', async (req, res) => {
  try {
    res.json({ success: true, data: await db.getActiveOffers() });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.get('/api/admin/offers', async (req, res) => {
  try {
    res.json({ success: true, data: await db.getOffers() });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.get('/api/announcements', async (req, res) => {
  try {
    res.json({ success: true, data: await db.getActiveAnnouncements() });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
app.get('/api/admin/announcements', async (req, res) => {
  try {
    res.json({ success: true, data: await db.getAnnouncements() });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
...
// ─── Production Static Serving for Google Cloud Run / Docker / Render ─────────
...
app.listen(PORT, () => {
  ...
  console.log(`📡 Database: ${isFirestoreConnected ? 'Google Cloud Firestore (Global Shared)' : 'Local JSON (Fallback)'}`);
});
